...
approach). Post-processing (Isat, Vf, Te...) will be added later.
...
class DAQ_1DViewer_Keithley2420(DAQ_Viewer_base):
    ...
    def ini_detector(self, controller=None):
        ...
        if self.is_master:
            ...
            # Force the voltage range explicitly (matches working script)
    ...
    def grab_data(self, Naverage=1, **kwargs):
        ...
        try:
            ...
            # 2420 is already ON from init_mesure
            ...
            for voltage in voltages:
                ...
                for _ in range(n_avg):
                    samples.append(self.controller_2420.measure())
                current = float(np.mean(samples))
                currents.append(current)

            # Turn grid off after acquisition (optional, can be commented)
